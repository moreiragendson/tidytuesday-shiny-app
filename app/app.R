...
# Data
load(url("https://raw.githubusercontent.com/moreiragendson/tidytuesday/main/data/all_weeks.RData"))
...
My contributions can be found on [GitHub](https://github.com/moreiragendson/tidytuesday), and you can use this Shiny app to explore my visualisations with links to code for each individual plot.
"),
htmltools::hr(),
...
server <- function(input, output) {
  ...
  img_path <- shiny::reactive({
    glue::glue("https://raw.githubusercontent.com/moreiragendson/tidytuesday/main/{week_data()$img_fpath}")
  })
  ...
  code_path <- shiny::reactive({
    glue::glue(
      "https://github.com/moreiragendson/tidytuesday/tree/main/{week_data()$code_fpath}"
    )
  })

  output$code_link <- shiny::renderText({
    glue::glue(
      '<b>Code is available at</b>: <a href="{code_path()}" target="_blank" style="word-break: break-all;">{code_path()}</a>.'
    )
  })
  ...
  r4ds_path <- shiny::reactive({
    if (week_data()$year == "2021") {
      ...
      glue::glue(
        "https://github.com/rfordatascience/tidytuesday/blob/main/data/{week_data()$year}/{reformat_week}/readme.md"
      )
    } else {
      glue::glue(
        "https://github.com/rfordatascience/tidytuesday/blob/main/data/{week_data()$year}/{week_data()$week}/readme.md"
      )
    }
  })

  output$r4ds_link <- shiny::renderText({
    glue::glue('<b>DSLC data source</b>: <a href="{r4ds_path()}"  target="_blank" style="word-break: break-all;">{r4ds_path()}</a>.')
  })
}
